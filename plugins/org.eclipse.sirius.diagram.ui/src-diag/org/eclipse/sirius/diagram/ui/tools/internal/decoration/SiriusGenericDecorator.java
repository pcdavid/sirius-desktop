...
public class SiriusGenericDecorator extends AbstractDecorator {
        ...
        if (editPart instanceof IDiagramElementEditPart) {
            ...
            for (SiriusDecorationDescriptorProvider decorationDescriptorProvider : decorationDescriptorProviders) {
                if (decorationDescriptorProvider.provides((IDiagramElementEditPart) editPart)) {
                    List<DecorationDescriptor> decorationDescriptors2 = decorationDescriptorProvider.getDecorationDescriptors((IDiagramElementEditPart) editPart);
                    if (decorationDescriptors2 != null) {
                        decorationDescriptors.addAll(decorationDescriptors2);
                    }
                }
            }
    ...
    private Map<Position, IFigure> computeGroupDecorationsFigure(Map<Position, List<DecorationDescriptor>> decosDescAtPosition) {
        // order the decoration for each position
        orderDecorationDescriptor(decosDescAtPosition);

        // initialize group decoration figure
        Dimension figureDimension = getFigureDimension();
        ...
        for (Position position : groupBoundsAtPosition.keySet()) {
            ...
            for (DecorationDescriptor decorationDescriptor : decoDescs) {
                ...
                // Rectangle groupBounds = groupFigure.getBounds();
     ...
     * @return
     */
    private Dimension getFigureDimension() {
    ...
    private class FigureWithLazyTooltip extends Figure {
        ...
        public FigureWithLazyTooltip(DecorationDescriptor decorationDescriptor) {
    ...
    private class ListDecorationFigure extends RectangleFigure {
        ...
        public ListDecorationFigure(List<DecorationDescriptor> decoDescriptors) {
            super();
        ...
        public IFigure getToolTip() {
            if (toolTip == null) {
                ...
                for (DecorationDescriptor decoDescriptor : decoDescriptors) {
                    composite.add(getDecorationFigureForTooltip(decoDescriptor));
