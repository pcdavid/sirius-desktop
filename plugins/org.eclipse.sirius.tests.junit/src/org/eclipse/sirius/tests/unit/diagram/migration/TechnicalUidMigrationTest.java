...
public class TechnicalUidMigrationTest extends SiriusTestCase {
    ...
    private static final String DREPRESENTATION_XMIID = "_sH154InWEeiSiblR-QI2Dg";

    private static final String DREPRESENTATION_UID = "_sGbkoInWEeiSiblR-QI2Dg";
    ...
    public void testMigrationIsNeededOnData() {
        ...
        assertTrue(xmiIdAttributes.contains(DREPRESENTATION_XMIID) && uidAttributes.contains(DREPRESENTATION_UID));
    }
    ...
    public void testTechnicalUidMigrationDone() {
        ...
        List<String> newUids = uidAttributes.stream().filter(s -> !DREPRESENTATION_UID.equals(s) && !initialXmiIdAttributes.contains(s)).collect(Collectors.toList());
        assertTrue("Migration should not introduce new uids", newUids.isEmpty());

        assertFalse("There should be no xmi:id for the DRepresentation",
                xmiIdAttributes.contains(DREPRESENTATION_UID) || xmiIdAttributes.contains(DREPRESENTATION_XMIID) || uidAttributes.contains(DREPRESENTATION_XMIID));
        assertTrue("The uid of the DRepresentation must be serialized in the uid tag", uidAttributes.contains(DREPRESENTATION_UID) && !uidAttributes.contains(DREPRESENTATION_XMIID));
        ...
        assertTrue("The DRepresentationDescriptor.repPath should contain the uid of the DRepresentation",
                dRepresentationDescriptor.getRepPath().getResourceURI().toString().contains(DREPRESENTATION_UID));
        assertEquals("Bad DRepresentation.uid. It should equals the xmiid of the DRepresentation.", DREPRESENTATION_UID, dRepresentationDescriptor.getRepresentation().getUid());
        ...
        assertEquals("GMF Diagram is no more able to retrieve its DDiagram, check the xmi:id to uid migration in post load.", dRepresentationDescriptor.getRepresentation(), gmfDiag.getElement());
