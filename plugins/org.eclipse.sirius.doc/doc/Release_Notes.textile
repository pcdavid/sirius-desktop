...
* <span class="label label-success">Added</span> An extension point:@org.eclipse.sirius.common.resource_sync_backend@ so that specific @AbstractResourceSyncBackend@ can be contributed by other plugins. These classes are responsible for notifying the session when an external change impacted on or several resources currently loaded.
