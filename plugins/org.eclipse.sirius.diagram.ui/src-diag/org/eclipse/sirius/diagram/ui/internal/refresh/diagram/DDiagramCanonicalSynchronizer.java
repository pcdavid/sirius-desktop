...
public class DDiagramCanonicalSynchronizer extends AbstractCanonicalSynchronizer {
    ...
    private void refreshSemantic() {
        if (gmfDiagram != null && gmfDiagram.getElement() != null) {
            ...
            canonicalSynchronizerResult.reconciliateOrphanedNodes();
            canonicalSynchronizerResult.collectNoteAttachedNode();
            canonicalSynchronizerResult.computeAttachedEdgeToNodes();
            canonicalSynchronizerResult.deleteOrphanedNodes();

            // 2. Refresh edges
            refreshConnections(gmfDiagram, canonicalSynchronizerResult);
            // reconciliation delete/create, compute deletion dependencies and confirm deletion
            canonicalSynchronizerResult.reconciliateOrphanedEdges();
            canonicalSynchronizerResult.computeAttachedEdgeToEdges();
            canonicalSynchronizerResult.collectNoteAttachedEdge();
            canonicalSynchronizerResult.deleteOrphanedEdges();

            // 3. Delete collected note attached to removed element
            canonicalSynchronizerResult.deleteOrphanedNotes();
     ...
     * 
     * <ul>
     * <li>Compute diffrence between Sirius and GMF model</li>
     * <li>Create all missing edge GMF and mark it in `canonicalSynchronizerResult`</li>
     * <li>Mark as orphaned (i.e to remove) in `canonicalSynchronizerResult` all GMF edge not present Sirius</li>
     * </ul>
     */
    private CanonicalSynchronizerResult refreshConnections(final Diagram diagram, CanonicalSynchronizerResult canonicalSynchronizerResult) {
        // Descritpion de l'algorithme :
        // Prend le descriptor tout les edges Sirius et enlève tout ceux qui existe :
        // // -> donc il reste tout les edges qui sont à créer (le manque de GMF à créer)
        // Prend tout les edges (sauf noteAttachments et lines) et enlève ceux qui ont un élément Sirius associé :
        // // -> donc il reste tout les edges orphelin (le suplus GMF à supprimer)
        ...
        // Remove NoteAttachments, Lines and already marked removed Edges
        existingLinks.removeAll(collectAllNoteAttachments(diagram));
        existingLinks.removeAll(collectAllLines(diagram));
        existingLinks.removeAll(canonicalSynchronizerResult.getOrphanedEdges());
                ...
                if (nextDiagramLink.getSource() != null && nextDiagramLink.getTarget() != null && nextDiagramLink.getSource().eContainer() != null
                    ...
                    while (linkDescriptorsIterator.hasNext()) {
                        ...
                        if (exist && diagramLinkSrc instanceof DEdge) {
                            // ---- true && ...
                            exist = exist && linkDescriptors.stream().noneMatch(input -> {
        ...
        canonicalSynchronizerResult.addOrphanedEdges(existingLinks);
    ...
    /**
     * Collect all links in Sirius model and return description of there links and create map between existing Sirius
