...
public class ValidationDecorationDescriptorProvider extends AbstractSiriusDecorationDescriptorProvider implements SiriusDecorationDescriptorProvider {
    ...
    private static Map<org.eclipse.gef.GraphicalEditPart, IDecorator> siriusDecorator = new HashMap<>();
    ...
    public void activate(IDecoratorTarget decoratorTarget, IDecorator decorator, org.eclipse.gef.GraphicalEditPart editPart) {
        ...
        if (view != editPart.getModel())
            DiagramPlugin.getDefault().logError("View from decoratorTarget.getAdapter(View.class) is different from View from decoratorTarget.getModel()"); //$NON-NLS-1$
        /// FIN DEBUG ///
        if (view != null) {
